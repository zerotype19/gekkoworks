...
Runs **during market hours** every **1 minute** during regular trading hours (RTH).

Responsibilities (in order):

1. Early exit gates:

   * Check market hours (`isMarketOpen`) → return if closed
   * Check risk gates (`canOpenNewTrade`) → return if blocked
2. If open positions exist:

   * Do **NOT** open new trades (v1 max positions = 1) → return early
...
**Cron Schedule:** `*/1 14-20 * * MON-FRI` (every 1 minute, 14:00-20:59 UTC, Mon-Fri)

---

### 6.3 `cron/monitorCycle.ts`

Runs every **1 minute** during regular trading hours (RTH) to monitor open positions and execute exits.

Responsibilities:

1. Early exit gate:

   * Check if any open/pending trades exist → return early if none (avoids unnecessary broker calls)
2. For all `ENTRY_PENDING` trades:

   * Check fills & transition to `OPEN` or `CANCELLED`
3. For all `CLOSING_PENDING` trades:

   * Check fills & transition to `CLOSED`
4. For all `OPEN` trades:

   * Call `engine/monitoring.evaluateOpenTrade`
   * If an exit trigger is signaled:

     * Call `engine/exits.executeExitForTrade`

This is the **heartbeat** of the system.

**Cron Schedule:** `1-59/1 14-20 * * MON-FRI` (every 1 minute, offset by 1 minute from trade cycle, 14:00-20:59 UTC, Mon-Fri)
