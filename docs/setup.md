...
crons = [
  ...
  "*/15 14-20 * * MON-FRI", # trade cycle (every 15 min during RTH)
  "*/2 14-20 * * MON-FRI"   # monitor cycle (every 2 min; infra limit approximation)
]
```

Cloudflare cron granularity is minute-based, so the “every 2 seconds” spec is conceptual; Cursor must emulate as frequently as infra allows (this is acceptable as long as the logic is written to assume frequent checks).
